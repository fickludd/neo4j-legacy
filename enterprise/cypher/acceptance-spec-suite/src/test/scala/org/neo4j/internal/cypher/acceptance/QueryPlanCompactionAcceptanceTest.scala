  ...
  test("Don't compact complex query") {
    ...
    val expectedPlan =
      """
        ...
        |+-------------------------+----------------+---------------------------+-----------------------+
        |""".stripMargin
    executeWith(expectedToSucceed, query, planComparisonStrategy = ComparePlansWithAssertion(_ should matchPlan(expectedPlan), expectPlansToFail = Configs.All - Configs.Cost3_3))
  }
