...
case class InterpretedPipeBuilder(recurse: LogicalPlan => Pipe,
  ...
  def onOneChildPlan(plan: LogicalPlan, source: Pipe): Pipe = {
    ...
    plan match {
      ...
      case LoadCSVPlan(_, url, variableName, format, fieldTerminator, legacyCsvQuoteEscaping, periodicCommit) =>
        LoadCSVPipe(source, format, buildExpression(url), variableName, fieldTerminator, legacyCsvQuoteEscaping, periodicCommit)(id = id)
