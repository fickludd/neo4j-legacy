...
import org.neo4j.cypher.internal.v3_4.expressions.{NodePattern, RelationshipPattern, VarDeclare, Variable}
...
case object nameAllPatternElements extends Rewriter {
  ...
  val namingRewriter: Rewriter = bottomUp(Rewriter.lift {
    case pattern: NodePattern if pattern.variable.isEmpty =>
      ...
      pattern.copy(variable = Some(VarDeclare(syntheticName)(pattern.position)))(pattern.position)
