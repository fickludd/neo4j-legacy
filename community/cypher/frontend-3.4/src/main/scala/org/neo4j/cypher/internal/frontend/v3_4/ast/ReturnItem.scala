...
final case class ReturnItems(
  ...
  def aliases: Set[VarDeclare] = items.flatMap(_.alias).toSet

  def passedThrough: Set[VarLike] = items.collect {
...
case class UnaliasedReturnItem(expression: Expression, inputText: String)(val position: InputPosition) extends ReturnItem {
  val alias = expression match {
    case i: VarLoad => Some(i.declare)
    case x: MapProjection => Some(x.name.declare)
    case _ => None
  }
...
object AliasedReturnItem {
  def apply(v:VarDeclare):AliasedReturnItem = AliasedReturnItem(VarLoad(v.name)(v.position), v)(v.position)
}

//TODO variable should not be a Variable. A Variable is an expression, and the return item alias isn't
case class AliasedReturnItem(expression: Expression, variable: VarDeclare)(val position: InputPosition) extends ReturnItem {
