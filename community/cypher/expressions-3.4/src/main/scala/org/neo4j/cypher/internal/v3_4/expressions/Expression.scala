...
object Expression {
  ...
  final case class TreeAcc[A](data: A, stack: Stack[Set[VarLike]] = Stack.empty) {
    def mapData(f: A => A): TreeAcc[A] = copy(data = f(data))

    def inScope(variable: VarLike) = stack.exists(_.contains(variable))
    def variablesInScope: Set[VarLike] = stack.toSet.flatten

    def pushScope(newVariable: VarLike): TreeAcc[A] = pushScope(Set(newVariable))
    def pushScope(newVariables: Set[VarLike]): TreeAcc[A] = copy(stack = stack.push(newVariables))
...
abstract class Expression extends ASTNode {
  ...
  def dependencies: Set[VarLike] =
    this.treeFold(TreeAcc[Set[VarLike]](Set.empty)) {
  ...
  def occurrences(variable: VarLike): Set[Ref[VarLike]] =
    this.treeFold(TreeAcc[Set[Ref[VarLike]]](Set.empty)) {
      case scope: ScopeExpression => {
        case acc =>
          val newAcc = acc.pushScope(scope.introducedVariables)
          (newAcc, Some((x) => x.popScope))
      }
      case occurrence: VarLike if occurrence.name == variable.name => acc => {
        val newAcc = if (acc.inScope(occurrence)) acc else acc.mapData(_ + Ref(occurrence))
        (newAcc, Some(identity))
      }
    }.data

  def copyAndReplace(variable: VarLike) = new {
    def by(replacement: => Expression): Expression = {
      ...
      self.endoRewrite(bottomUp(Rewriter.lift {
        case occurrence: VarLike if replacedOccurences(Ref(occurrence)) => replacement
      }))
    }
  }
  ...
  def inputs: Seq[(Expression, Set[VarLike])] =
    this.treeFold(TreeAcc[Seq[(Expression, Set[VarLike])]](Seq.empty)) {
      case scope: ScopeExpression=>
        acc =>
          val newAcc = acc.pushScope(scope.introducedVariables)
